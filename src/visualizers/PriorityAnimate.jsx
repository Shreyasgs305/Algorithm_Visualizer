...
function PriorityAnimate({ data }) {
  ...
  useEffect(() => {
    ...
    data.GanttChart.forEach((item, index) => {
      ...
      timeline.add({
        targets: `.priority-block-${index}`,
        scaleX: [0, 1],
        duration: (duration / total) * 2000,
      });
    });
  }, [data, total]);
  ...
  return (
    <div style={{ width: "85%", margin: "40px auto" }}>
      <div
        style={{ display: "flex", border: "2px solid black", height: "60px" }}
      >
        {data.GanttChart.map((item, index) => {
          ...
          return (
            <div
              key={index}
              className={`priority-block-${index}`}
              style={{
                flex,
                background: "#2196F3",
                color: "white",
...
export default PriorityAnimate;
